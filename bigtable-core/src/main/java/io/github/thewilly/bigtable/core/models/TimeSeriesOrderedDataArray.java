 ...
 *
 */
public class TimeSeriesOrderedDataArray implements VersionableDataArray {

  /** The Log. */
  final Logger log = LoggerFactory.getLogger(TimeSeriesOrderedDataArray.class);

  private final LinkedList<DataVersion> _dataVersions;
  private final int _numberOfVersionsToStore;

  /** Instantiates a new Time series ordered data. */
  public TimeSeriesOrderedDataArray() {
   ...
   */
  public TimeSeriesOrderedDataArray(int numberOfVersionsToStore) {
   ...
   * @return the all data
   */
  public Stream<DataVersion> getAllVersions() {
   ...
   */
  @Override
  public DataVersion getMostRecentVersion() {
   ...
   */
  @Override
  public DataVersion addVersion(DataVersion data) {
    log.debug("Adding {} to the time series ordered data.", data);
    DataVersion previousData = null;
    try {
      previousData = _dataVersions.getLast();
      previousData.invalidate();
      } catch (NoSuchElementException e) {
      log.debug("Adding {} as the first element the time series ordered data.", data);
    }
